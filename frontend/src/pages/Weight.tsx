...
const Weight: React.FC = () => {
  ...
  const fetchOrders = useCallback(async () => {
    try {
      ...
      console.error("Error fetching weighbridge orders:", error);
    } finally {
      setLoading(false);
    }
    ...
    try {
      switch (actionType) {
        ...
        case "exit":
          ...
          break;
        default:
          console.warn(`Unsupported weighbridge action: ${actionType}`);
          return;
      }
      ...
      console.error("Error performing weighbridge action:", error);
    }
  };
        ...
        {viewMode === "cards" ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            {filteredOrders.map((order) => (
              <OrderCard key={order._id} order={order} onActionClick={handleActionClick} showActions={true} />
            ))}
          </div>
        ) : (
          <OrdersTable orders={filteredOrders} loading={loading} onActionClick={handleActionClick} />
        )}
...
const OrdersTable: React.FC<{
  ...
  const columns = [
    ...
    },
    {
      ...
      title: "Actions",
      render: (_: any, record: Order) => (
        <Button variant="ghost" size="sm" icon={Eye} onClick={() => onActionClick("view", record._id)} />
      ),
    },
  ];
